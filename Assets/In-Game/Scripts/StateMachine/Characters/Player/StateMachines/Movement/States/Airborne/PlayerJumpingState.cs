    ...
    public class PlayerJumpingState : PlayerAirborneState
    {
        ...
        private void Jump()
        {
            ...
            if(Physics.Raycast(downwardsRayFromCapsuleCenter,out RaycastHit hit,jumpData.JumpToGroundRayDistance,stateMachine.Player.LayerData.GroundLayer,QueryTriggerInteraction.Ignore))
            {
                ...
                if (IsMovingUp())
                {
                    float forceModifier = jumpData.JumpForceModifierOnSlopeUp.Evaluate(groundAngle);
                    jumpForce.x *= forceModifier;
                    jumpForce.z *= forceModifier;
                }
                if (IsMovingDown())
                {
                    float forceModifier = jumpData.JumpForceModifierOnSlopeDown.Evaluate(groundAngle);
